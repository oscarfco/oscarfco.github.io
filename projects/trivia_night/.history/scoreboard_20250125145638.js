...
function showResults() {
    ...
    // Store round results before the animations
    ...
    // After all animations (around line 3000ms delay)
    setTimeout(() => {
        drawRankingGraph();
    }, 3000);
...
function drawRankingGraph() {
    ...
    const colors = ['#2ecc71', '#e74c3c', '#3498db', '#f1c40f', '#9b59b6', '#1abc9c', '#e67e22', '#34495e'];
            ...
            <svg class="graph-svg">
                ${teamNames.map((team, teamIndex) => {
                    const pathPoints = roundHistory.map((round, roundIndex) => {
                        const teamRank = round.findIndex(r => r.team === team) + 1;
                        const x = (roundIndex * 100) + 50;
                        const y = (teamRank - 1) * 50 + 25;
                        return `${x},${y}`;
                    }).join(' L ');
