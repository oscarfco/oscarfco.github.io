...
function showResults() {
    ...
    // After showing score additions, sort to new positions
    setTimeout(() => {
        const newStandings = previousStandings
            .sort((a, b) => b.totalScore - a.totalScore);

        // Update positions with animation
        newStandings.forEach(({ team, totalScore }, newIndex) => {
            const teamElement = rankings.querySelector(`[data-team="${team}"]`);
            teamElement.style.transform = `translateY(${newIndex * (teamElement.offsetHeight + 10)}px)`;
            teamElement.innerHTML = `
                ${newIndex + 1}. ${team} - ${totalScore} points
                <span class="score-addition"></span>
            `;
        });
    }, 2500);
