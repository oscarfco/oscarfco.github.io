...
function updateTeamList() {
    ...
    Object.entries(teams).forEach(([team, data]) => {
        ...
        div.innerHTML = `
            ...
            <span style="color: ${teamColors[team]}">${team}</span>
        `;
...
function startGame() {
    ...
    scoreBoard.innerHTML = `
        ...
        ${Object.entries(teams).map(([team, data]) => `
            <div class="team-row">
                <div class="team-info" style="color: ${teamColors[team]}">${team}</div>
                <div class="team-score">${data.score}</div>
                <div class="score-input-container">
                    <input type="number" class="score-input" data-team="${team}" value="0">
                </div>
            </div>
        `).join('')}
    `;
    ...
    // Add stopwatch HTML
    const stopwatchHTML = `
        <div class="stopwatch-container">s
            <div class="stopwatch">
...
function updateScores() {
    console.log("Team Colors:", teamColors); // Debug log
    const scoreContainer = document.getElementById('scoreContainer');
    scoreContainer.innerHTML = '';
    
    Object.entries(teams).forEach(([teamName, teamData]) => {
        const teamColor = teamColors[teamName] || '#FFFFFF'; // Fallback to white if no color
        console.log(`Team: ${teamName}, Color: ${teamColor}`); // Debug log
        ...
        scoreDiv.innerHTML = `
            <div class="team-info">
                ${teamData.image ? 
                    `<img src="${teamData.image}" alt="${teamName}" class="team-image">` : 
                    `<div class="team-image-placeholder">${teamName[0].toUpperCase()}</div>`
                }
                <span class="team-name" style="color: ${teamColor}; text-shadow: 1px 1px 0px #000;">${teamName}</span>
            </div>
            <div class="score-controls">
                <button onclick="updateTeamScore('${teamName}', -1)">-</button>
                <span class="score">${teamData.score}</span>
                <button onclick="updateTeamScore('${teamName}', 1)">+</button>
            </div>
        `;
