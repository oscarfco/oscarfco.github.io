...
function drawRankingGraph() {
    ...
    const yScale = (pos) => {
        ...
        const spacing = graphHeight / totalPositions; // e.g. 300 - 0 for pure scale
        return pos * spacing; // returns something in [0..300]
      };
    ...
    let graphHTML = `
        ...
        <div class="graph-grid">
            <div class="graph-y-axis" style="padding: ${padding.top}px 10px ${padding.bottom}px 10px">
                ${teamNames.map((_, i) => `
                    <div class="y-label" style="position: absolute; top: ${yScale(i) - 8}px">${i + 1}</div>
                `).join('')}
            </div>
                ...
                ${teamNames.map((team, teamIndex) => {
                    ...
                    return `
                        ...
                        ${roundHistory.map((round, roundIndex) => {
                            ...
                            return `
                                <circle 
                                    cx="${xScale(roundIndex)}" 
                                cy="${padding.top + yScale(position)}"  
                                    r="4" 
