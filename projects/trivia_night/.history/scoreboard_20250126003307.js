...
function addTeam() {
    ...
    if (teamName && !(teamName in teams)) {
        ...
        if (file) {
            ...
            // Show confirmation dialog when no image is uploaded
            if (confirm('Do you want to proceed without a team image?')) {
                teams[teamName] = {
                    score: 0,
                    image: null
                };
        ...
        div.innerHTML = `
            <div class="team-image-container">
                ${data.image ? 
                    ...
                    `<div class="team-image-placeholder">${team.charAt(0).toUpperCase()}</div>`
                }
            </div>
            <span>${team}</span>
        `;
