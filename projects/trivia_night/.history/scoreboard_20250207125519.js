...
function startGame() {
    ...
    // Add stopwatch HTML
    const stopwatchHTML = `
        <div class="stopwatch-container" style="position: fixed; top: 5px; right: 5px; z-index: 1000;">
            <div class="stopwatch">
...
function endGame() {
    // Hide the modal
    document.getElementById('confirmEndGameModal').style.display = 'none';
    ...
    const finalRankings = Object.entries(teams)
        .sort(([,a], [,b]) => b.score - a.score)
    ...
    finalRankingsDiv.innerHTML = finalRankings
        .map((ranking, index) => `
            <div>${ranking.team}: ${ranking.score} points</div>
        `).join('');
    ...
    function singleBeep() {
        ...
        setTimeout(() => {
            ...
            if (beepCount < count) {
                ...
                // After all beeps are done, play random MP3
                playRandomSound();
            }
        }, 200);
    }
...
function playRandomSound() {
    const sounds = [
        "sounds/Hawk Tuah Sound Effect.mp3",
        "sounds/Kamala Harris Shares Her Favorite Curse Word With NowThis.mp3",
        "sounds/Trump_ They're eating the dogs, the cats.mp3",
        // Add all your sound files here
    ];
    
    const randomSound = sounds[currentRound-1];
