...
function startGame() {
    ...
    // Add stopwatch HTML
    const stopwatchHTML = `
        <div class="stopwatch-container" style="transform: translate(${window.innerWidth - 250}px, 100px);">
            <div class="stopwatch">
    ...
    // Insert stopwatch and initialize drag functionality
    document.body.insertAdjacentHTML('afterbegin', stopwatchHTML);
    initDraggableStopwatch();
    ...
    if (startBtn.textContent === 'Start Round') {
        ...
        timerInterval = setInterval(() => {
            ...
            if (timeRemaining <= 0) {
                ...
                // Optional: Play a sound or show an alert when timer ends
...
function initDraggableStopwatch() {
    ...
    const initialY = (window.innerHeight - stopwatch.offsetHeight) / 2;
    stopwatch.style.transform = `translate(${initialX}px, ${initialY}px)`;

    function onMouseDown(e) {
        if (e.target.tagName === 'BUTTON' || e.target.closest('.stopwatch-controls')) {
            return;
        }
        ...
        stopwatch.style.cursor = 'grabbing';
    }
    ...
    function onMouseMove(e) {
        ...
        // Update position
        stopwatch.style.transform = `translate(${x}px, ${y}px)`;
    }

    function onMouseUp() {
        isDragging = false;
        stopwatch.style.cursor = 'grab';
    }

    // Add event listeners
    stopwatch.addEventListener('mousedown', onMouseDown);
    ...
    window.addEventListener('resize', () => {
        ...
        if (rect.right > window.innerWidth) {
            ...
            stopwatch.style.transform = `translate(${newX}px, ${rect.top}px)`;
        }
        if (rect.bottom > window.innerHeight) {
            const newY = window.innerHeight - rect.height;
            stopwatch.style.transform = `translate(${rect.left}px, ${newY}px)`;
        }
    });
} 