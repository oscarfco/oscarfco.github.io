...
function drawRankingGraph() {
    ...
    const width = roundHistory.length * 100;
    const height = (teamNames.length + 1) * 50;
    const padding = 30;
    ...
    let graphHTML = `
        ...
        <div class="graph-grid">
            ...
            <svg class="graph-svg" viewBox="0 0 ${width + padding * 2} ${height}">
                <g transform="translate(${padding}, 0)">
                    ${teamNames.map((team, teamIndex) => {
                        const pathPoints = roundHistory.map((round, roundIndex) => {
                            const position = round.indexOf(team);
                            const x = roundIndex * (width / (roundHistory.length - 1));
                            const y = position * (height / (teamNames.length - 1));
